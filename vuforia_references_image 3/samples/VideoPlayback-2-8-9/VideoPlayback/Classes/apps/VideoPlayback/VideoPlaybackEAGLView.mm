...
//動画選択のコード/動画のステータスを指定/表示させる動画をいれる(9→3)
- (void) prepare {
    NSLog(@"番号3");
...
// the screen.
//
// *** QCAR will call this method periodically on a background thread ***（確認）
- (void)renderFrameQCAR
{
    ...
    for (int i = 0; i < numActiveTrackables; ++i) {
        ...
        if (playerIndex == 0){
            ...
            NSLog(@"offTargetTrackingEnabled=%d",offTargetTrackingEnabled);
            if (offTargetTrackingEnabled) {
                NSLog(@"番号8a2a");
            ...
            if (!strcmp(imageTarget.getName(), "chips")){
                targetIndex = 1;}
            else if (!strcmp(imageTarget.getName(), "tarmac")){
                targetIndex = 2;}
             */
